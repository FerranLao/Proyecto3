...
router.post("/getevent",(req, res, next)=>{
  const {id}=req.body
  Events.findById(id).populate("game creator party").then(e=>{
    res.json(e)})
})
...
router.post("/joinparty",(req,res,next)=>{
  const {id} = req.body
  Events.findByIdAndUpdate(id,{$push:{party:req.user._id}}).populate("game event party creator").then(e=>res.json(e))
})
